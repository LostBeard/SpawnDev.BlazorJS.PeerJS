...
namespace SpawnDev.BlazorJS.PeerJS
{
    ...
    public class EventEmitter : JSObject
    {
        ...
        /// <param name="type"></param>
        /// <param name="listener"></param>
        public void Off(string type, Action listener)
        {
            if (!CallBackInfos.TryGetValue(listener, out CallBackInfo? info)) return;
            info.RefCount--;
            Off(type, info.Callback);
            if (info.RefCount <= 0)
            {
                CallBackInfos.Remove(listener);
                info.Callback.Dispose();
        ...
        /// Remove an event 
        /// </summary>
        public void Off<T1>(string type, Action<T1> listener)
        {
            if (!CallBackInfos.TryGetValue(listener, out CallBackInfo? info)) return;
            info.RefCount--;
            Off(type, info.Callback);
        ...
        /// Remove an event 
        /// </summary>
        public void Off<T1, T2>(string type, Action<T1, T2> listener)
        {
            if (!CallBackInfos.TryGetValue(listener, out CallBackInfo? info)) return;
            info.RefCount--;
            Off(type, info.Callback);
